...
public class NonAxisAllignedRectangle implements IPrimitive{
	...
	public boolean isInsidePrimitive(Point p) {
		return triangle1.isInsidePrimitive(p) || triangle2.isInsidePrimitive(p) || isOnLine(p, a, c);
	}
	
	private boolean isOnLine(Point p, Point l1, Point l2) {
		return Distance(l1,p)+Distance(p,l2)==Distance(l1,l2);
	}

	private int Distance(Point l1, Point l2) {
		double x = l1.x-l2.x;
		double y = l1.y-l2.y;
		return (int) Math.sqrt(Math.abs(x*x+y*y));
	}
